...
library(Hmisc)
library(rphast)
trfn = np(paste("tree_datada_sem_ilhas", sep=""))
moref(trfn)
tr = read.newick(file="tree_datada_sem_ilhas")
#######################################################
# Geography file
# Notes:
...
geofile<-read_PHYLIP_data("Phyllip_sem_ilhas.txt")
geogfn=geofile
geogfn = np(paste("Phyllip_sem_ilhas.txt", sep=""))
moref(geogfn)
...
There was substantial interest in the listserv, so I've created one, and added those who were interested who I remembered. It is invite-only so that we don't get robospam. If I forgot you, or you are interested, email me at matzke@nimbios.org to be added. Once you are added, you can post by emailing: biogeobearsATgooglegroups.com .
...
Even while I've been moving, I have gotten many emails about BioGeoBEARS. I am happy to answer them individually, but it might benefit people if there was an email listserv. Email me (matzke@nimbios.org) if you would like me to put you on such a listserv. It would be used for (1) BioGeoBEARS updates and announcements and (2) answers to questions.
...
1. Download the older, 2012 version of optimx from here: http://cran.r-project.org/src/contrib/Archive/optimx/ . Save it someplace you can easily find it again.
